...
parser.add_argument("--results", type=eval, default=False)
...
if args.results:
    results_path = os.path.join(folder_models, "results", f"{args.dataset}_{model_name}_results.joblib")
else:
    results_path = None
    
model = training.train_net(model, model_name, X_train, y_train, params, args.dataset, folder_models, X_val, y_val, history_path)
training.evaluation(model, X_train, X_val, y_train, y_val, results_path, type = 'classification')
